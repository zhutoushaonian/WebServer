...
http_conn::HTTP_CODE http_conn::process_read(){
    ...
    bool hasMore = true;
    // std::cout << "****************************" << std::endl;
    // std::cout << m_read_buf.returnString() << std::endl;
    // std::cout << "****************************" << std::endl;
    while(hasMore){
        ...
        switch (m_check_state)
        {
            ...
            case CHECK_STATE_HEADER:
            {
                ...
                if(crlf){
                    ...
                    // todo
                    int size = crlf - m_read_buf.begin();// 这里扩容了，导致了迭代器失效
                    if(colon != crlf){
                       // 解析头部
                       parse_headers(m_read_buf.peek(), colon, crlf);
                        ...
                        // std::cout << "CRLF:" << crlf - m_read_buf.begin() << std::endl;
                    ...
                    // std::cout << m_read_buf.testretrieveAsString() << std::endl;
                    // std::cout << m_read_buf.returnReadIndex() << std::endl;
                    // std::cout << crlf - m_read_buf.begin() << std::endl;
                    crlf = m_read_buf.begin() + size;
                    m_read_buf.retrieveUntil(crlf + 2);
                }else{
                    hasMore = false;
                }
            ...
            case CHECK_STATE_CONTENT:
            {
                ...
                // std::cout << m_read_buf.readableBytes() << std::endl;
                // std::cout << "****************************" << std::endl;
                // std::cout << m_read_buf.returnString() << std::endl;
                // std::cout << "****************************" << std::endl;
                // std::cout << m_read_buf.utilRetrieveAsString() << std::endl;
